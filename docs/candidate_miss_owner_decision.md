...
captured Medium component whose parent Sparse component is not an eligible Candidate Miss
...
No Medium-area threshold, distance merge, component consolidation, or new
density definition is introduced. A low-capture Sparse parent below 7,500 km²
does not suppress its Medium-core flag. Sparse remains 25%, Medium remains 40%,
and the `<20%` capture boundary remains provisional.
